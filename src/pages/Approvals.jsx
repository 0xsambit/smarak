...
const PAGE_SIZE = 10;
const TYPE_OPTIONS = ["CONSERVATION", "INCIDENT", "REPORT", "BUDGET"];
...
const submittedByName = (approval) => {
	if (!approval.submittedBy) {
		return "Unknown";
	}
...
const Approvals = () => {
	...
	const initializeContext = useCallback(async () => {
		...
		try {
			...
			setRole(meResponse.data?.role || null);
			setCurrentUserId(
				((meResponse.data?._id || meResponse.data?.id || "")).trim(),
			);
	...
	const handleRestore = async (approval) => {
		if (!canArchive) {
								...
								{!loading
									? filteredApprovals.map((approval) => (
											<tr
												...
												<td className="px-4 py-3 text-stone-700">
													{approval.isPriority ? "High" : "Normal"}
												</td>
												<td className="px-4 py-3">
													<div className="flex flex-wrap items-center gap-2">
														{!showArchived ? (
															<>
																{canEditApproval(approval) ? (
																	<button
																		onClick={() =>
																			openEdit(approval)
																		}
																...
																{canReview && approval.status === "PENDING" ? (
																	<>
																		<button
																			onClick={() =>
																				handleReview(approval, "APPROVED")
																			}
																		...
																		<button
																			onClick={() =>
																				handleReview(approval, "REJECTED")
																			}
							...
							<div className="grid grid-cols-1 gap-3 md:grid-cols-2">
								...
								<label className="inline-flex items-center gap-2 rounded-md border border-stone-300 px-3 py-2 text-sm text-stone-700">
									<input
										...
										onChange={(event) =>
											onFormChange("isPriority", event.target.checked)
										}
									/>
