...
import { ClerkAuthGuard } from '../src/common/guards/clerk-auth.guard.js';
import { Approval, ApprovalStatus, ApprovalType } from '../src/schemas/approval.schema.js';
import { Conservation, ConservationStatus } from '../src/schemas/conservation.schema.js';
import { Footfall } from '../src/schemas/footfall.schema.js';
import { Incident, IncidentSeverity, IncidentStatus, IncidentType } from '../src/schemas/incident.schema.js';
import { ProtectionStatus, RiskLevel, Site } from '../src/schemas/site.schema.js';
import { User, UserRole } from '../src/schemas/user.schema.js';

class TestClerkAuthGuard {
  canActivate(context) {
    const req = context.switchToHttp().getRequest();
    const encodedActor = req.headers['x-test-user'];

    if (!encodedActor || Array.isArray(encodedActor)) {
      throw new UnauthorizedException('Missing x-test-user header');
    }

    req.user = JSON.parse(Buffer.from(encodedActor, 'base64').toString('utf8'));
    return true;
  }
}

const encodeActor = (actor) => Buffer.from(JSON.stringify(actor)).toString('base64');

const withActor = (req, actor) => req.set('x-test-user', encodeActor(actor));

const buildActor = (role, overrides = {}) => {
...
const buildSitePayload = (overrides = {}) => ({
  name: 'Humayun Tomb',
  state: 'Delhi',
  district: 'New Delhi',
  coordinates: {
...
describe('CRUD API regression suite', () => {
  ...
  beforeAll(async () => {
    ...
    siteModel = moduleRef.get(getModelToken(Site.name));
    incidentModel = moduleRef.get(getModelToken(Incident.name));
    conservationModel = moduleRef.get(getModelToken(Conservation.name));
    approvalModel = moduleRef.get(getModelToken(Approval.name));
    userModel = moduleRef.get(getModelToken(User.name));
    footfallModel = moduleRef.get(getModelToken(Footfall.name));
  });
  ...
  const persistActor = async (actor) => {
